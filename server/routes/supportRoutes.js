...
router.get("/console-overview", protect, async (req, res) => {
  try {
    ...
    // Fetch all support/admin agents
    ...
    // CSAT scores from recent orders (simulated from order ratings)
    ...
    // Per-agent stats (chat messages sent today)
    ...
    const agentStats = agents.map(a => {
      ...
      return {
        ...
        lastActive: myMessages.length > 0
          ...
          : null,
        isOnline: myMessages.length > 0 && (Date.now() - new Date(myMessages[myMessages.length - 1].createdAt).getTime()) < 15 * 60 * 1000 // active in last 15 min
      };
    });
    ...
    // Issue breakdown (simulated from order statuses and categories)
    const categories = {};
    allOrders.forEach(o => {
