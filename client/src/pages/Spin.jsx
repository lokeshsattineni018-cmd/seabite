...
const Spin = ({ isOpen, onClose }) => {
  ...
  // ✅ New: Check if user has already spun when modal opens
  useEffect(() => {
    const verifySpinStatus = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/spin/status`, { withCredentials: true });
        if (res.data.hasSpun) {
          setResult(res.data.previousResult); // Set existing result if they already spun
        }
      } catch (err) {
        console.error("Could not verify spin status:", err);
      } finally {
        ...
        >
          ...
          {checkingStatus ? (
            <div className="flex flex-col items-center py-12">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm">Checking spin status...</p>
            </div>
          ) : (
            <>
              <div className="text-center mb-6">
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Spin & Win</h2>
                <p className="text-slate-600 dark:text-slate-400 text-sm">Get instant discounts on your order</p>
              </div>

              {/* Show the wheel ONLY if no result exists yet */}
              {!result && (
                ...
                >
                  {result.result === "BETTER_LUCK" ? (
                    <div className="p-6 bg-slate-100 dark:bg-slate-800 rounded-2xl text-center">
                      <p className="text-xl font-bold text-slate-900 dark:text-white mb-1">Better luck next time!</p>
                      <p className="text-slate-600 dark:text-slate-400 text-sm">Try again later</p>
                    </div>
                  ) : (
                    <div className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-2xl border-2 border-green-500/20 text-center">
                      ...
                      <p className="text-4xl font-black text-slate-900 dark:text-white mb-1">{result.discountValue}% OFF</p>
                      <p className="text-green-700 dark:text-green-300 text-sm font-semibold">Applied to your next order!</p>
                    </div>
                  )}
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs text-center mt-4">
                One spin per account • Expires in 24 hours
              </p>
            </>
          )}
        </motion.div>
