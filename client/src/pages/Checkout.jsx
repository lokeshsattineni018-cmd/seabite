...
const DEFAULT_DELIVERY_ADDRESS = {
    fullName: "", phone: "", houseNo: "", street: "", city: "", state: "", zip: "",
};
...
export default function Checkout() {
  ...
  const handleUpdateQty = (id, change) => {
    const updated = cart.map((item) => item._id === id ? { ...item, qty: item.qty + change } : item).filter((item) => item.qty > 0);
    setCart(updated);
    saveCart(updated);
    refreshCartCount();
  ...
  const placeOrder = async () => {
    ...
    const orderDetails = {
        amount: grandTotal,
        items: cart.map((item) => ({ productId: item._id, name: item.name, price: item.price, qty: item.qty, image: item.image })),
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] pt-24 md:pt-32 pb-12 md:pb-20 px-4 md:px-8 font-sans text-slate-900 dark:text-slate-200 relative overflow-hidden transition-colors duration-500">
      ...
      <AnimatePresence>
        {isAddressModalOpen && (
            <AddressModal onClose={() => setIsAddressModalOpen(false)} onSave={saveNewAddress} currentAddress={deliveryAddress} isDarkMode={isDarkMode} />
        )}
      </AnimatePresence>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10 relative z-10">
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
             <span className="text-[10px] md:text-xs font-bold text-blue-600 dark:text-blue-500 uppercase tracking-widest mb-1 md:mb-2 block">Secure Checkout</span>
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-800 p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 md:mb-8 gap-4">
              <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                <div className="p-2 md:p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl"><FiMapPin size={20} /></div>
                Delivery Address
              </h3>
              <button onClick={() => setIsAddressModalOpen(true)} className="w-full sm:w-auto text-[10px] md:text-xs font-bold text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 px-5 py-2.5 rounded-full hover:bg-blue-600 hover:text-white transition-all tracking-wide uppercase">
              ...
              {deliveryAddress.street ? (
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2 md:gap-3">
                    <p className="font-bold text-base md:text-lg text-slate-900 dark:text-white">{deliveryAddress.fullName}</p>
                    <span className="hidden sm:block h-4 w-[1px] bg-slate-300 dark:bg-slate-600" />
                    <p className="text-slate-600 dark:text-slate-400 font-medium text-xs md:text-sm">{deliveryAddress.phone}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-slate-700 dark:text-slate-300 font-semibold text-xs md:text-sm flex items-center gap-2">
                        <FiHome className="text-blue-500" size={14}/> {deliveryAddress.houseNo}
                    </p>
                ...
                <p className="text-slate-400 dark:text-slate-500 text-xs md:text-sm py-4 text-center font-medium">Please add a delivery address to proceed.</p>
              )}
              
              <div className={`mt-4 md:mt-6 inline-flex items-center gap-2 text-[9px] md:text-[10px] font-bold uppercase tracking-wider px-3 md:px-4 py-2 rounded-lg border ${isDeliveryAllowed ? 'text-emerald-700 bg-emerald-50 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-900/30' : 'text-red-700 bg-red-50 border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-900/30'}`}>
                {isDeliveryAllowed ? <><FiCheckCircle size={14} /> Serviceable Area</> : <><FiXCircle size={14} /> Area Not Serviceable</>}
              </div>
            </div>
          </motion.section>

          {/* Payment Method Section */}
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-800 p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none">
             <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-6 md:mb-8 flex items-center gap-3">
                <div className="p-2 md:p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl"><FiCreditCard size={20} /></div>
                Payment Method
             </h3>
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-500">Pay when your order reaches your door</p>
                        </div>
                    </div>
                </div>
             </div>
          ...
          {/* Items Section */}
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-slate-800 p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none">
            <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-6 md:mb-8 flex items-center gap-3">
               <div className="p-2 md:p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl"><FiShoppingBag size={20} /></div>
               Your Items
            </h3>
                ...
                <AnimatePresence mode="popLayout">
                    ...
                    >
                        ...
                        <div className="flex-1 text-center sm:text-left">
                           ...
                           <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-medium mt-1">₹{item.price.toFixed(2)} / unit</p>
                        </div>
                        <div className="flex items-center justify-between w-full sm:w-auto gap-4 md:gap-6">
                            <div className="flex items-center bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-1">
                                <button onClick={() => handleUpdateQty(item._id, -1)} className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"><FiMinus size={12} /></button>
                                <span className="w-7 md:w-8 text-center font-bold text-xs md:text-sm text-slate-900 dark:text-white">{item.qty}</span>
                                <button onClick={() => handleUpdateQty(item._id, 1)} className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"><FiPlus size={12} /></button>
                            </div>
                            <p className="font-bold text-slate-900 dark:text-white text-sm md:text-base min-w-[70px] md:min-w-[80px] text-right">₹{(item.price * item.qty).toFixed(2)}</p>
                            <button onClick={() => handleRemoveItem(item._id)} className="text-slate-300 hover:text-red-500 transition-colors p-2"><FiTrash2 size={16} /></button>
                        </div>
                    </motion.div>
                    ))}
                </AnimatePresence>
            ...
            >
              ...
              <div className="mb-6">
                  ...
                  <div className="flex gap-2">
                      <div className="relative flex-1">
                          ...
                          />
                          {isCouponApplied && (
                              <button onClick={handleRemoveCoupon} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-red-500">
                                  <FiX size={16} />
                              </button>
                          )}
                      </div>
                  ...
                  {couponMessage && (
                      <p className={`text-[10px] font-bold mt-2 ml-1 ${couponMessage.type === 'success' ? 'text-emerald-500' : 'text-red-500'}`}>
                          {couponMessage.text}
                      </p>
                  )}
              </div>
              ...
              <div className="space-y-3 md:space-y-4 text-xs md:text-sm font-medium">
                ...
                <div className="flex justify-between text-slate-500 dark:text-slate-400">
                    ...
                    <span className={`font-bold ${deliveryCharge === 0 ? "text-emerald-500 uppercase" : "text-slate-900 dark:text-white"}`}>
                        {deliveryCharge === 0 ? "Free" : `₹${deliveryCharge.toFixed(2)}`}
                    </span>
                </div>
                <div className="flex justify-between text-slate-500 dark:text-slate-400">
                    <span>Applicable Tax (GST 5%)</span>
                    <span className="text-slate-900 dark:text-white font-bold">₹{gst.toFixed(2)}</span>
                </div>
              </div>
              
              <div className="my-5 md:my-6 h-px bg-slate-100 dark:bg-slate-700"></div>
              <div className="flex justify-between items-end mb-6 md:mb-8">
                <span className="text-xs md:text-sm font-bold text-slate-500 dark:text-slate-400">Grand Total</span>
                <span className="text-2xl md:text-3xl font-extrabold text-blue-600 dark:text-blue-400">₹{grandTotal.toFixed(2)}</span>
              </div>
              ...
              {itemTotal < 1000 && (
                <div className="mb-6 p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/20">
                    ...
                    <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${(itemTotal / 1000) * 100}%` }} className="h-full bg-blue-600" />
                    </div>
              ...
              <button
                ...
                className={`w-full py-3.5 md:py-4 rounded-xl font-bold uppercase tracking-wide text-[10px] md:text-xs shadow-lg transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-3 ${isDeliveryAllowed ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-blue-600 dark:hover:bg-blue-50 shadow-slate-900/20' : 'bg-slate-200 dark:bg-slate-700 text-slate-400'}`}
              >
                {loading ? <div className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : (isDeliveryAllowed ? <><FiCheckCircle size={16}/> {paymentMethod === "COD" ? "Place COD Order" : "Pay & Place Order"}</> : "Location Not Supported")}
              </button>
            </motion.div>
...
function AddressModal({ onClose, onSave, currentAddress, isDarkMode }) {
    ...
    const fetchAddress = async (lat, lon) => {
        ...
        try {
            ...
            if (data) {
                ...
                const detectedCity = addr.city || addr.town || addr.village || addr.hamlet || addr.suburb || addr.neighbourhood || addr.county || "";
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-2 md:p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-slate-800 rounded-[1.5rem] md:rounded-[2.5rem] w-full max-w-2xl shadow-2xl border border-white dark:border-white/5 overflow-hidden flex flex-col max-h-[95vh] transition-colors">
                <form onSubmit={handleFormSearch} className="p-4 md:p-6 border-b border-slate-100 dark:border-white/5 flex items-center gap-3 md:gap-4 bg-white dark:bg-slate-800 z-10">
                    <div className="relative flex-1">
                        <FiSearch className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search location..." className="w-full pl-10 md:pl-11 pr-4 py-2.5 md:py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white text-xs md:text-sm font-semibold focus:ring-2 focus:ring-blue-500 transition-all" />
                    </div>
                    <button type="button" onClick={onClose} className="p-2.5 md:p-3 text-slate-400 hover:text-red-500 transition-colors bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700"><FiXCircle size={20} /></button>
                </form>

                <div className="relative h-48 md:h-72 bg-slate-100 shrink-0">
                    <div ref={mapContainerRef} className="w-full h-full" />
                    <button onClick={detectLocation} className="absolute bottom-4 right-4 md:bottom-6 md:right-6 bg-white dark:bg-slate-800 p-2.5 md:p-3 rounded-xl shadow-lg text-blue-600 dark:text-blue-400 z-[1000] hover:scale-105 active:scale-95 transition-all border border-slate-200 dark:border-slate-700">
                        <FiTarget size={20} />
                    </button>
                </div>
                ...
                <div className="p-5 md:p-8 space-y-4 md:space-y-5 overflow-y-auto no-scrollbar">
                    ...
                    <div className="space-y-1.5">
                         <label className="text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 ml-1">House No. / Flat / Building</label>
                         <div className="relative">
                            <FiHome className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                            <input type="text" placeholder="e.g. Flat 101, Sea View Apts" value={form.houseNo} onChange={e => setForm({...form, houseNo: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 pl-11 p-3 md:p-3.5 rounded-xl text-slate-900 dark:text-white text-xs md:text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
                         </div>
                    </div>
