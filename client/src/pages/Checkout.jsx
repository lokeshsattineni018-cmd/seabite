...
import {
  FiMapPin, FiCheckCircle, FiMinus, FiPlus, FiTrash2, FiShoppingBag, 
  FiTag, FiX, FiCreditCard, FiLoader, FiShield, FiTarget
} from "react-icons/fi";
...
export default function Checkout() {
  ...
  // ✅ AUTO-FETCH REWARD FROM MONGODB (No localStorage for rewards)
  useEffect(() => {
    const fetchUserReward = async () => {
      ...
      try {
        const res = await axios.post(`${API_URL}/api/coupons/validate`, {
          email: email,
          cartTotal: itemTotal,
          isAutoCheck: true // Backend uses this to find user's MongoDB record
        });
        ...
        if (res.data.success) {
          ...
          setCouponMessage({ type: "success", text: "Reward discount applied automatically!" });
        }
      } catch (err) {
        console.log("No active reward found in MongoDB.");
      }
    };
    fetchUserReward();
  }, [itemTotal]);
  ...
  const placeOrder = async () => {
    ...
    if (!deliveryAddress.zip) { setModal({ show: true, message: "Please add an address first.", type: "error"}); return; }
    setLoading(true);

    const orderDetails = { 
      amount: grandTotal, items: cart.map(i => ({ productId: i._id, name: i.name, price: i.price, qty: i.qty, image: i.image })), 
      shippingAddress: deliveryAddress, itemsPrice: itemTotal, taxPrice: gst, shippingPrice: deliveryCharge, discount, paymentMethod: "COD" 
    };
    ...
    try {
      ...
      setModal({ show: true, message: err.response?.data?.message || "Order Error", type: "error" }); 
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <h2 className="text-4xl md:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-none">Order <span className="text-blue-600">Summary</span></h2>
          </motion.div>
          ...
          <section className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-white/5 shadow-xl">
            ...
            {deliveryAddress.street ? (
              <div className="space-y-3 bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-100 dark:border-white/5 text-slate-900 dark:text-white">
            ...
            ) : <p className="text-slate-400 text-sm text-center">Please add a delivery address to proceed.</p>}
          </section>

          <section className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border shadow-xl">
             <h3 className="text-xl font-bold mb-8 flex items-center gap-3 text-slate-900 dark:text-white"><div className="p-2.5 bg-blue-50 text-blue-600 rounded-xl"><FiCreditCard size={20} /></div> Payment Method</h3>
          ...
          <section className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border shadow-xl">
            <h3 className="text-xl font-bold mb-8 flex items-center gap-3 text-slate-900 dark:text-white"><div className="p-2.5 bg-blue-50 text-blue-600 rounded-xl"><FiShoppingBag size={20} /></div> Your Items</h3>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-32 space-y-6">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] shadow-2xl border border-slate-100 relative overflow-hidden">
              <h3 className="text-lg font-bold mb-6 uppercase tracking-wide text-slate-900 dark:text-white">Payment Details</h3>
              <div className="mb-6">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <FiTag className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input type="text" placeholder="Promo Code" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} disabled={isCouponApplied || verifyingCoupon} className="w-full pl-9 pr-4 py-3 bg-slate-50 dark:bg-slate-900 border rounded-xl text-sm font-semibold uppercase outline-none focus:border-blue-500 text-slate-900 dark:text-white" />
                  ...
                  <button onClick={handleApplyCoupon} disabled={isCouponApplied || !couponCode || verifyingCoupon} className="bg-slate-900 dark:bg-blue-600 text-white px-4 rounded-xl font-bold text-xs hover:bg-blue-600">{verifyingCoupon ? <FiLoader className="animate-spin" /> : "Apply"}</button>
                </div>
              ...
              <div className="space-y-4 text-sm font-medium text-slate-900 dark:text-white">
                <div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Subtotal</span><span className="font-bold">₹{itemTotal.toFixed(2)}</span></div>
                ...
                <div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Shipping</span><span className={`font-bold ${deliveryCharge === 0 ? "text-emerald-500 uppercase" : ""}`}>{deliveryCharge === 0 ? "FREE" : `₹${deliveryCharge}`}</span></div>
              </div>
              ...
              <div className="mt-4 flex items-center justify-center gap-2 text-[10px] text-slate-400 uppercase tracking-widest font-bold"><FiShield className="text-emerald-500" /> Secure SSL Connection</div>
            </motion.div>
          </div>
        </div>
      </div>
...
function AddressModal({ onClose, onSave, currentAddress, isDarkMode }) {
  const mapRef = useRef(null); const mapInstance = useRef(null); const markerInstance = useRef(null);
  ...
  useEffect(() => {
    ...
    mapInstance.current = L.map(mapRef.current, { zoomControl: false, attributionControl: false }).setView(initialPos, 13);
    ...
    markerInstance.current.on("dragend", (e) => { fetchAddress(e.target.getLatLng().lat, e.target.getLatLng().lng); });
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-8 space-y-5 overflow-y-auto text-slate-900 dark:text-white">
          ...
          <input type="text" placeholder="House/Flat No." value={form.houseNo} onChange={(e) => setForm({ ...form, houseNo: e.target.value })} className="w-full bg-slate-50 border dark:bg-slate-900 p-3.5 rounded-xl text-sm" />
          <input type="text" placeholder="Street/Area" value={form.street} readOnly className="w-full bg-slate-100 dark:bg-slate-700 border p-3.5 rounded-xl text-xs opacity-70" />
          <div className="grid grid-cols-3 gap-5">
             <input type="text" placeholder="City" value={form.city} readOnly className="bg-slate-100 dark:bg-slate-700 border p-3.5 rounded-xl text-sm" />
             <input type="text" placeholder="State" value={form.state} readOnly className="bg-slate-100 dark:bg-slate-700 border p-3.5 rounded-xl text-sm" />
             <input type="text" placeholder="Zip" value={form.zip} onChange={(e) => setForm({ ...form, zip: e.target.value })} className="border p-3.5 rounded-xl text-sm dark:bg-slate-900" />
          </div>
          <button disabled={isDetecting || !form.fullName || !form.phone} onClick={() => onSave(form)} className="w-full py-4 bg-slate-900 dark:bg-blue-600 text-white rounded-xl font-bold uppercase tracking-wide text-xs disabled:opacity-50">Confirm Address</button>
        </div>
      </motion.div>
    </div>
  );
