  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#0f172a] transition-colors duration-500">
        ...
        <p className="text-slate-500 font-medium tracking-widest text-sm uppercase">Loading Fresh Catch...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#0f172a] text-center px-4">
        ...
        <h2 className="text-3xl font-serif font-bold text-slate-900 dark:text-white mb-2">Item Not Found</h2>
        <p className="text-slate-500 mb-6">We couldn't find the fresh catch you were looking for.</p>
        <Link to="/products" className="px-6 py-3 bg-blue-600 text-white rounded-full text-sm font-bold hover:bg-blue-700 transition-colors">
            Back to Market
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] relative font-sans pt-24 md:pt-28 pb-12 md:pb-20 px-4 md:px-6 transition-colors duration-500 overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {/* Back Navigation */}
        <Link to="/products" className="inline-flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-white transition-colors mb-6 md:mb-10 text-xs md:text-sm font-bold tracking-wide group">
          ...
          >
            ...
            <div className="flex items-center gap-4 md:gap-6 mb-6 md:mb-8">
              <p className="text-2xl md:text-3xl font-sans font-bold text-slate-900 dark:text-white">
                 ₹{basePrice.toFixed(2)}
                 <span className="text-base md:text-lg text-slate-400 font-normal ml-1">/{product.unit || 'kg'}</span>
              </p>
            ...
            <div className="mb-8 md:mb-10">
              <div className="flex gap-6 md:gap-8 border-b border-slate-200 dark:border-slate-700 mb-4 md:mb-6 overflow-x-auto no-scrollbar">
                <button 
                  ...
                  className={`pb-3 text-xs md:text-sm font-bold uppercase tracking-wider transition-all relative whitespace-nowrap ${activeTab === "desc" ? "text-blue-600 dark:text-blue-400" : "text-slate-400 hover:text-slate-600 dark:text-slate-500"}`}
                >
                  Description
                  {activeTab === "desc" && <motion.div layoutId="tabLine" className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-full" />}
                </button>
                <button 
                  onClick={() => setActiveTab("shipping")}
                  className={`pb-3 text-xs md:text-sm font-bold uppercase tracking-wider transition-all relative whitespace-nowrap ${activeTab === "shipping" ? "text-blue-600 dark:text-blue-400" : "text-slate-400 hover:text-slate-600 dark:text-slate-500"}`}
                >
                  Delivery
                  {activeTab === "shipping" && <motion.div layoutId="tabLine" className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-full" />}
                </button>
              </div>
                  ...
                  {activeTab === "desc" ? (
                    ...
                    >
                       <div className="flex items-start gap-3">
                          ...
                          <div>
                              <p className="text-xs md:text-sm font-bold text-slate-900 dark:text-white">Express Delivery</p>
                              <p className="text-xs md:text-sm text-slate-500">Delivered within 1-2 days of order confirmation.</p>
                          </div>
                       </div>
                       <div className="flex items-start gap-3">
                          <FiInfo className="text-blue-600 mt-1 shrink-0" />
                          <div>
                              <p className="text-xs md:text-sm font-bold text-slate-900 dark:text-white">Packaging</p>
                              <p className="text-xs md:text-sm text-slate-500">Vacuum sealed to ensure maximum freshness.</p>
                          </div>
                       </div>
                    </motion.div>
                  )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6 md:pt-8 border-t border-slate-100 dark:border-white/10">
              ...
              <div className="flex items-center justify-between bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full px-2 py-1.5 w-full sm:w-48 shadow-sm">
                ...
                <span className="font-bold text-slate-900 dark:text-white text-base md:text-lg">{qty}</span> 
              ...
              {/* Add to Cart Button */}
              <motion.button
                whileHover={!isAdded && product.stock !== 'out' ? { scale: 1.02 } : {}}
                whileTap={!isAdded && product.stock !== 'out' ? { scale: 0.98 } : {}}
              ...
              >
                {isAdded ? (
                  <>
                    <FiCheck size={18} md:size={20} /> Added
                  </>
                ) : product.stock === 'out' ? (
                  "Sold Out"
                ) : (
                  <>
                    <FiShoppingBag size={16} md:size={18} /> ₹{totalPrice} — Add
                  </>
                )}
              </motion.button>
            </div>
