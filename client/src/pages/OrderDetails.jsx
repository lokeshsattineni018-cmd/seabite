    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] text-slate-900 dark:text-slate-200 pt-24 pb-12 px-4 md:px-8 font-sans">
        ...
        {showCancelConfirm && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white dark:bg-slate-800 p-6 rounded-2xl max-w-sm w-full shadow-2xl text-slate-900"
            >
              ...
              <select
                ...
                className="w-full p-3 mb-4 bg-slate-50 dark:bg-slate-900 border rounded-xl text-sm font-semibold outline-none transition-all"
              >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl font-bold bg-slate-100 dark:bg-slate-700"
                >
                  Go Back
                </button>
                <button
      ...
      <div className="max-w-4xl mx-auto">
            ...
            <div className="relative w-full max-w-2xl mx-auto text-slate-900">
              <div
                className={`absolute top-[14px] left 0 w-full h-[3px] rounded-full z-0 ${
                  isCancelled ? 'bg-red-100' : 'bg-slate-200'
                }`}
              />
                ...
                {isCancelled ? (
                  <>
                    <div className="flex flex-col items-center">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center border-4 border-white bg-slate-400 shadow-sm">
                    ...
                    <div className="flex flex-col items-center">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center border-4 border-white bg-red-600 shadow-md animate-pulse">
                  ...
                  statusSteps.map((step, index) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center border-4 border-white dark:border-slate-800 ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-6 bg-slate-50/20 dark:bg-slate-900/10 text-slate-900">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[9px] font-black text-blue-600 bg-blue-100 px-2.5 py-1 rounded uppercase tracking-widest">
                  {order.paymentMethod}
                </span>
              </div>
              <div className="space-y-2">
                ...
                {isCancelled && (
                  <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 rounded-xl">
                    <p className="text-[10px] text-red-600 font-bold uppercase italic tracking-tight">
                      Cancellation Reason:
                    </p>
          ...
          <div className="p-6 border-t dark:border-white/5">
            ...
            <div className="space-y-3">
              {order.items.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-4 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5"
                >
                  <img
                    src={`${API_URL}/uploads/${item.image.split('/').pop()}`}
                    alt={item.name}
                    className="w-14 h-14 object-cover rounded-xl bg-slate-50 p-1"
                  />
        ...
        <div className="mt-8 flex flex-wrap gap-4 justify-center">
          <button
            onClick={() => navigate('/')}
            className="px-8 py-3 bg-white dark:bg-slate-800 border border-slate-200 rounded-2xl font-bold text-xs flex items-center gap-2 shadow-sm hover:bg-slate-50 transition-colors text-slate-900 dark:text-white"
          >
            <FiHome /> Home
          </button>
          <button
            onClick={handlePrintInvoice}
            className="px-8 py-3 bg-white dark:bg-slate-800 border rounded-2xl font-bold text-xs flex items-center gap-2 shadow-sm hover:bg-slate-50 transition-colors text-slate-900 dark:text-white"
          >
            <FiFileText /> Invoice
          </button>
          {canCancel && (
            <button
              ...
              className="px-8 py-3 bg-red-50 text-red-600 border border-red-100 rounded-2xl font-black text-xs hover:bg-red-100 transition-all"
            >
              Cancel Order
            </button>
          )}
          <button
            ...
            className="px-10 py-3 bg-slate-900 text-white rounded-2xl font-bold text-xs flex items-center gap-2 hover:scale-105 transition-all shadow-lg shadow-slate-900/20"
          >
