...
const EditableInfoRow = ({ icon: Icon, label, initialValue, fieldKey, onSave }) => {
    ...
    return (
        <motion.div 
            layout
            className={`
                group relative flex items-center gap-3 md:gap-5 p-4 md:p-6 rounded-2xl transition-all duration-300
                ${isEditing 
        ...
        >
            {/* Icon Column */}
            <div className={`
                shrink-0 p-2.5 md:p-3 rounded-full transition-colors duration-300
                ${isEditing 
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="relative">
                    {isEditing && !isEmailField ? (
                        ...
                        <p className={`text-base md:text-lg font-medium truncate ${isEmailField ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-slate-200'}`}>
                            {initialValue || <span className="text-slate-400 italic text-sm">Not provided</span>}
                        </p>
                    )}
                </div>
            </div>
                ...
                {!isEmailField && (
                    <AnimatePresence mode='wait'>
                        {isEditing ? (
                            ...
                            >
                                <button onClick={handleSave} disabled={loading} className="p-2 bg-emerald-500 text-white rounded-full hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 transition-all">
                                    {loading ? <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" /> : <FiCheck size={16} md:size={18} />}
                                </button>
                                <button onClick={handleCancel} className="p-2 bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-300 rounded-full hover:bg-red-50 hover:text-red-500 transition-all">
        ...
        <div className="max-w-3xl mx-auto -mt-16 md:-mt-24 relative z-10 px-4 md:px-6 mb-12 md:mb-20 font-sans"> 
            ...
            <div className="flex flex-col items-center mb-8 md:mb-10 text-center">
                <div className="relative">
                    ...
                    <div className="absolute bottom-0.5 right-0.5 md:bottom-1 md:right-1 bg-emerald-500 text-white p-1 md:p-1.5 rounded-full border-2 md:border-4 border-white dark:border-slate-800 shadow-md">
                        <FiCheck size={12} md:size={14} strokeWidth={3} />
                    </div>
                </div>
                
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-4 md:mt-6">
                    <h1 className="text-2xl md:text-3xl font-serif font-bold text-slate-900 dark:text-white tracking-tight">{name}</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 text-xs md:text-sm font-medium">SeaBite Member since 2025</p>
                </motion.div>
            </div>
            ...
            >
                ...
                <div className="flex flex-col gap-1 md:gap-2">
                    <EditableInfoRow icon={FiUser} label="Full Name" fieldKey="name" initialValue={name} onSave={handleFieldSave} />
                    <EditableInfoRow icon={FiMail} label="Email Address" fieldKey="email" initialValue={email} onSave={handleFieldSave} />
                    <EditableInfoRow icon={FiSmartphone} label="Phone Number" fieldKey="phone" initialValue={phone} onSave={handleFieldSave} />
                </div>
            </motion.div>
        </div>
    );
