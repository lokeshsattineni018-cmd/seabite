      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-white dark:bg-[#0a1625] rounded-[2rem] shadow-2xl overflow-hidden p-8"
          >
            <button
              onClick={handleClose}
            ...
            <h2 className="text-3xl font-serif text-center text-slate-900 dark:text-white mb-2">
              Spin & Win
            </h2>
            <p className="text-center text-slate-500 dark:text-slate-400 text-sm mb-6">
              Spin once to win a special coupon. It will be used automatically at checkout.
            </p>
            ...
            <div className="flex flex-col items-center gap-4">
              <button
                ...
                className="w-40 h-40 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold text-lg flex items-center justify-center shadow-xl disabled:opacity-60"
              >
                {spinning ? "Spinning..." : result ? "Done" : "SPIN"}
              </button>
    ...
    <div className="relative h-screen w-screen left-1/2 -translate-x-1/2 overflow-hidden bg-slate-900 z-20">
      ...
      <div className="relative z-20 h-full flex flex-col items-center justify-center text-center px-4">
        <motion.div
          ...
          className="space-y-6"
        >
          <p className="text-blue-200 font-mono text-xs md:text-sm tracking-[0.5em] uppercase drop-shadow-md">
            The Ocean&apos;s Finest
          </p>
          <h1 className="text-[15vw] md:text-[12vw] leading-none font-serif text-white opacity-95 select-none drop-shadow-2xl">
            SEABITE
          </h1>
          <motion.div
            initial={{ width: 0 }}
          ...
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Link to="/products">
              <button className="mt-8 px-10 py-4 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full font-bold tracking-widest text-xs hover:bg-white hover:text-black transition-all duration-500 shadow-xl">
                Shop Now
              </button>
            </Link>
          </motion.div>
      ...
      <motion.div className="flex whitespace-nowrap" style={{ x }}>
        {[...Array(4)].map((_, i) => (
          <div key={i} className="flex items-center">
            <span className="text-sm font-bold uppercase tracking-[0.2em] mx-8 text-blue-200">
              Fresh Catch Daily
            </span>
            <Star className="w-3 h-3 text-blue-500" fill="currentColor" />
            <span className="text-sm font-bold uppercase tracking-[0.2em] mx-8 text-white">
              Sustainable
            </span>
...
const categories = [
  {
    title: "Premium Fish",
    img: "/fish.png",
    bg: "from-blue-100/80 to-white dark:from-blue-900/40 dark:to-[#0a1625]",
  },
  {
    title: "Jumbo Prawns",
    img: "/prawn.png",
    bg: "from-indigo-100/80 to-white dark:from-indigo-900/40 dark:to-[#0a1625]",
  },
  {
    title: "Live Crabs",
    img: "/crab.png",
    bg: "from-cyan-100/80 to-white dark:from-cyan-900/40 dark:to-[#0a1625]",
  },
];
        ...
        {categories.map((cat, i) => (
          ...
          >
              ...
              <motion.img
                ...
                className={`object-contain drop-shadow-2xl transition-all duration-700 ${
                  hovered === i
                    ...
                    : "w-24 h-24 opacity-50 grayscale"
                }`}
              />
            </div>
            <div className="absolute bottom-0 left-0 w-full p-8 z-20 bg-gradient-to-t from-white via-white/80 dark:from-black dark:via-[#0a1625]/80 to-transparent">
              ...
              <div
                className={`flex items-center gap-2 text-blue-600 dark:text-blue-300 text-sm font-bold uppercase tracking-widest mt-2 overflow-hidden transition-all duration-500 ${
                  hovered === i ? "max-h-10 opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                Explore <ArrowRight />
              </div>
        ...
        <div className="relative z-10 bg-white p-6 rounded-xl shadow-lg transform rotate-2 md:rotate-0">
          <div className="text-center">
            ...
            <div className="flex gap-2 text-slate-900 font-mono font-black text-3xl">
              ...
              :
              <span className="bg-slate-100 px-2 rounded text-red-600">
  ...
  useEffect(() => {
    axios.get(`${API_URL}/api/products`).then((res) => {
      ...
      if (filterType === "Fish") {
        ...
        filtered = all
          .filter((p) => p.category === "Prawn" || p.category === "Crab")
          .slice(0, 4);
      }
        ...
        <div className="flex justify-between items-end mb-8 border-b border-gray-200 dark:border-gray-800 pb-4">
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            {filterType === "Fish" ? (
              ...
              <Anchor className="text-orange-500" />
            )}
            {title}
          </h2>
          <Link
            ...
            <Link to={`/products/${p._id}`} key={p._id}>
              <div className="group bg-white dark:bg-[#1e293b] border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 h-full flex flex-col">
                <div className="relative h-40 md:h-48 bg-slate-50 dark:bg-[#0f172a] flex items-center justify-center p-4">
                  ...
                  />
                  {p.trending && (
                    <span className="absolute top-2 left-2 bg-yellow-400 text-yellow-900 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">
                      BESTSELLER
                    </span>
                  )}
                </div>
    ...
    <section className="py-20 px-6 transition-colors duration-300 relative">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row bg-[#0f172a] dark:bg-white rounded-3xl overflow-hidden shadow-2xl relative z-10 group">
        ...
        <div className="w-full md:w-[40%] bg-[#0f172a] dark:bg-blue-50 p-8 md:p-12 flex flex-col justify-center items-center text-center text-white dark:text-slate-900 relative">
          ...
          <Link to="/products">
            <button className="w-full py-4 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-xl font-bold uppercase tracking-wider text-sm hover:scale-105 transition-transform shadow-lg">
              Shop Now
            </button>
          </Link>
        </div>
              ...
              <div className="bg-white dark:bg-[#0e1d30] border border-gray-200 dark:border-white/5 rounded-[2rem] p-6 hover:bg-white/80 dark:hover:bg-[#112238] transition-colors backdrop-blur-sm shadow-sm hover:shadow-xl">
                <div className="h-[220px] mb-6 flex items-center justify-center relative">
                  <img
                    src={getImageUrl(p.image)}
                    alt={p.name}
                    className="w-48 h-48 object-contain drop-shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500"
                  />
                </div>
          ...
          {loading ? (
            ...
            reviews.map((r, i) => (
              <motion.div
                key={i}
                whileHover={{ y: -10 }}
                className="bg-white/80 dark:bg-[#0e1d30]/90 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 relative shadow-sm hover:shadow-xl transition-all duration-300 backdrop-blur-sm"
              >
        ...
        >
          ...
          <Link to="/products" className="inline-block">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-12 py-5 bg-[#0f172a] dark:bg_white text-white dark:text-slate-900 rounded-full font-bold text-lg transition-all shadow-xl hover:shadow-2xl"
            >
              Start Your Order
            </motion.button>
          </Link>
