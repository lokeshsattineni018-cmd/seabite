          ...
          >
            ...
            {!result ? (
              <button
                onClick={handleSpin}
                disabled={spinning}
                className="w-full py-5 bg-slate-900 text-white font-bold rounded-[1.5rem] shadow-xl hover:bg-slate-800 transition-all active:scale-95"
              >
              ...
              <div className="w-full space-y-6">
                ...
                <Link
                  to="/products"
                  className="block w-full py-5 bg-blue-600 text-white font-bold rounded-[1.5rem] shadow-xl"
                >
                  Shop Now
                </Link>
              </div>
    ...
    <div className="relative h-screen w-full overflow-hidden bg-slate-900 z-20">
      ...
      <div className="relative z-20 h-full flex flex-col items-center justify-center text-center px-4">
        <motion.div
          ...
          className="space-y-6"
        >
          <p className="text-blue-200 font-mono text-xs md:text-sm tracking-[0.5em] uppercase">
            The Ocean&apos;s Finest
          </p>
          <h1 className="text-[15vw] md:text-[12vw] leading-none font-serif text-white opacity-95">
            SEABITE
          </h1>
          <Link to="/products">
            <button className="mt-8 px-10 py-4 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full font-bold tracking-widest text-xs hover:bg-white hover:text-black transition-all shadow-xl">
              Shop Now
            </button>
          </Link>
        </motion.div>
...
const categories = [
  {
    title: "Premium Fish",
    img: "/fish.png",
    bg: "from-blue-100/80 to-white dark:from-blue-900/40",
  },
  {
    title: "Jumbo Prawns",
    img: "/prawn.png",
    bg: "from-indigo-100/80 to-white dark:from-indigo-900/40",
  },
  {
    title: "Live Crabs",
    img: "/crab.png",
    bg: "from-cyan-100/80 to-white dark:from-cyan-900/40",
  },
];
        ...
        {categories.map((cat, i) => (
          <Link
            ...
            className={`relative rounded-[2rem] overflow-hidden transition-all duration-700 ${
              hovered === i ? "flex-[3]" : "flex-[1] opacity-70"
            }`}
          >
            <div
              className={`absolute inset-0 bg-gradient-to-b ${cat.bg} z-0`}
            />
            ...
            <div className="absolute bottom-0 p-8 z-20 w-full bg-gradient-to-t from-white dark:from-black">
              <h3 className="font-serif text-slate-900 dark:text-white text-2xl">
                {cat.title}
              </h3>
            </div>
          </Link>
  ...
  return (
    <section className="py-10 px-4">
      <div className="max-w-6xl mx-auto bg-gradient-to-r from-red-600 to-rose-600 rounded-2xl p-12 text-white flex flex-col md:flex-row items-center justify-between shadow-2xl">
        <div>
          <h2 className="text-4xl md:text-5xl font-black italic mb-4 uppercase tracking-tighter">
            TODAY&apos;S CATCH
          </h2>
          <p className="text-red-50 text-xl font-serif">
            Order above ₹1699 & use{" "}
            <span className="bg-white text-red-600 px-3 py-1 rounded font-mono font-bold">
              SEABITE10
            </span>
          </p>
        </div>
        <Link
          to="/products"
          className="px-10 py-4 bg-white text-red-600 rounded-xl font-bold uppercase tracking-widest shadow-lg mt-6 md:mt-0"
        >
          Grab Deal
        </Link>
      </div>
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (filterType === "Fish")
          setProducts(all.filter((p) => p.category === "Fish").slice(0, 4));
        else if (filterType === "Shellfish")
          setProducts(
            all
              .filter((p) => ["Prawn", "Crab"].includes(p.category))
              .slice(0, 4)
          );
      })
      .catch((err) => {
  ...
  return (
    <section className="py-12 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold mb-8 border-b pb-4 text-slate-900 dark:text-white">
          {title}
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {products.map((p) => (
            <Link
              to={`/products/${p._id}`}
              key={p._id}
              className="group bg-white dark:bg-[#1e293b] rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all border dark:border-gray-800"
            >
    ...
    <section className="py-20 px-6">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row bg-[#0f172a] rounded-3xl overflow-hidden shadow-2xl">
        <div className="w-full md:w-[60%] h-64 md:h-auto">
          <img src="/20offer.png" className="w-full h-full object-cover" />
        </div>
        <div className="w-full md:w-[40%] p-12 text-center text-white flex flex-col justify-center items-center">
          ...
          <h3 className="text-xl uppercase tracking-widest mb-6 opacity-80">
            Flat Discount
          </h3>
          <div className="w-full border-2 border-dashed border-white/20 p-4 rounded-xl mb-6">
            <p className="text-2xl font-mono font-bold text-emerald-400 uppercase tracking-widest">
              SEABITE20
            </p>
          </div>
          <Link
            to="/products"
            className="w-full py-4 bg-white text-slate-900 rounded-xl font-bold uppercase text-sm shadow-lg"
          >
            Shop Now
          </Link>
        </div>
    ...
    <section className="py-24 overflow-hidden bg-slate-50 dark:bg-[#081220]">
      ...
      >
        {products.map((p, i) => (
          <div
            key={`${p._id}-${i}`}
            className="w-[300px] bg-white dark:bg-[#0e1d30] p-6 rounded-[2rem] shadow-sm border dark:border-gray-800"
          >
            <img
              src={`${API_URL}/${p.image}`}
    ...
    <section className="py-24 px-6 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
        {reviews.map((r, i) => (
          <div
            key={i}
            className="bg-white dark:bg-[#0e1d30] p-8 rounded-[2rem] border dark:border-gray-800 text-left shadow-sm"
          >
...
const Footer = () => {
  return (
    <footer className="bg-slate-900 text-white py-24 text-center">
      ...
      <Link to="/products">
        <button className="px-12 py-5 bg-white text-slate-900 rounded-full font-bold text-lg shadow-xl">
          Start Your Order
        </button>
      </Link>
    </footer>
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
          {[...Array(10)].map((_, i) => (
            <motion.div
              ...
              className="absolute text-blue-500/20 top-[20%]"
              style={{ top: `${i * 10}%` }}
            >
