...
const CATEGORY_DATA = {
    "All": { title: "Fresh Catch", desc: "Sustainable seafood.", gradient: "from-blue-500/10 via-cyan-400/5 to-transparent", accent: "text-blue-600" },
    "Fish": { title: "Premium Fish", desc: "Freshwater cuts.", gradient: "from-cyan-500/10 via-blue-400/5 to-transparent", accent: "text-cyan-600" },
    "Prawn": { title: "Jumbo Prawns", desc: "Perfect for grilling.", gradient: "from-orange-500/10 via-amber-400/5 to-transparent", accent: "text-orange-600" },
    "Crab": { title: "Live Crabs", desc: "Soft-shell delicacies.", gradient: "from-red-500/10 via-rose-400/5 to-transparent", accent: "text-red-600" }
};
    ...
    return (
        <motion.button
            ...
            className={`relative h-8 md:h-10 flex items-center justify-center rounded-full transition-all duration-300 font-bold uppercase tracking-wider text-[9px] md:text-[10px]
                ${status === "success" ? "w-8 md:w-10 bg-emerald-500 text-white" : "px-3 md:px-5 bg-slate-900 dark:bg-blue-600 text-white hover:opacity-90"}
            `}
        >
            <AnimatePresence mode="wait">
                ...
                {status === "loading" && (
                    <motion.div key="loading" className="w-3 h-3 md:w-4 md:h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                )}
                {status === "success" && (
...
export default function Products() {
    ...
    return (
        <div className="min-h-screen bg-[#fafafa] dark:bg-[#0b1221] pt-24 md:pt-32 pb-12 md:pb-20 px-3 md:px-12 transition-colors duration-500 overflow-x-hidden font-sans">
            ...
            <div className={`fixed top-0 left-0 w-full h-[50vh] bg-gradient-to-b ${currentTheme.gradient} opacity-30 pointer-events-none -z-10`} />
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6">
                    <AnimatePresence mode="popLayout">
                        {loading ? (
                            [...Array(6)].map((_, i) => (
                                <div key={i} className="h-56 md:h-80 rounded-[1.5rem] bg-slate-200 dark:bg-slate-800 animate-pulse" />
                            ))
                        ) : (
                            displayed.map((p, idx) => (
                                <TiltCard key={p._id} onClick={() => navigate(`/products/${p._id}`)}>
                                    ...
                                    >
                                        ...
                                        <div className="p-3 md:p-5 flex-grow flex flex-col justify-between bg-white dark:bg-slate-800/50">
                                            <div>
                                                <span className="text-[9px] md:text-[10px] font-semibold uppercase tracking-wider text-slate-400 block mb-1">{p.category}</span>
                                                <h3 className="text-sm md:text-lg font-semibold text-slate-800 dark:text-slate-100 leading-snug line-clamp-2">{p.name}</h3>
                                            </div>
                                            
                                            <div className="mt-3 md:mt-4 flex items-center justify-between gap-1">
                                                <div className="flex flex-col">
                                                    <span className="text-sm md:text-xl font-bold text-slate-900 dark:text-white">₹{Number(p.basePrice)}</span>
                                                    <span className="text-[9px] font-medium text-slate-400">/{p.unit || 'kg'}</span>
                                                </div>
                ...
                {!loading && displayed.length === 0 && (
                    <div className="text-center py-20">
                        ...
                        <h2 className="text-lg font-medium dark:text-white">No catches found</h2>
