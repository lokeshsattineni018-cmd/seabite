...
const ReviewModal = ({ isOpen, onClose, product, existingReview, token, API_URL, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // We now rely on the parent (Order.jsx) to trigger the professional PopupModal 
      if (onSuccess) {
          onSuccess(); 
      }
      onClose(); 
    } catch (err) {
      // 🟢 You can replace this with a local error state if you don't want alerts here either
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`text-2xl transition-transform hover:scale-110 ${rating >= star ? 'text-amber-400' : 'text-slate-300'}`}
                >
