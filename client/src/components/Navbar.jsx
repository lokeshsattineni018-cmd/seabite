...
import { FiUser, FiShoppingBag, FiSearch, FiLogOut, FiPackage, FiGrid, FiBell, FiMenu, FiX, FiSun, FiMoon, FiChevronDown } from "react-icons/fi"; 
  ...
  return (
        ...
        <div className="flex items-center gap-2 md:gap-6">
          ...
          >
            <button className="flex items-center gap-1.5 text-sm font-medium text-slate-700 dark:text-blue-100 hover:text-blue-600 dark:hover:text-white transition-colors py-2">
                Shop <FiChevronDown className={`transition-transform duration-300 ${showShop ? 'rotate-180' : ''}`} />
            </button>
            <AnimatePresence>
                {showShop && (
                    <motion.div 
          ...
          <div className="hidden md:flex items-center">
            {user ? (
                <div className="relative" onMouseEnter={() => setShowProfile(true)} onMouseLeave={() => setShowProfile(false)}>
                    <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-3 cursor-pointer group ml-4">
                        <div className="relative">
                            ...
                            {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 border-2 border-white dark:border-slate-900 rounded-full animate-pulse" />}
                        </div>
                    ...
                    <AnimatePresence>
                        ...
                        >
                            <div className="p-5 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5">
                                <p className="text-[10px] font-black text-blue-600 dark:text-blue-300 uppercase tracking-widest mb-1">Authenticated</p>
                            ...
                            <div className="py-2">
                                ...
                                <DropdownItem icon={<FiBell />} text="Notifications" onClick={() => navigate("/notifications")} badge={unreadCount} />
                                {user.role === "admin" && <DropdownItem icon={<FiGrid />} text="Admin Dashboard" onClick={() => navigate("/admin/dashboard")} />}
                            </div>
                ...
                <motion.button onClick={() => googleLogin()} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
    ...
    <AnimatePresence>
        {mobileMenuOpen && (
            ...
            >
                <button className="absolute top-6 right-6 text-slate-900 dark:text-white text-3xl" onClick={() => setMobileMenuOpen(false)}><FiX /></button>
                <div className="space-y-6 pb-12">
                    ...
                    <div className="bg-slate-100 dark:bg-white/10 rounded-xl p-4 flex items-center border border-slate-200 dark:border-white/10">
                         ...
                         <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleSearch}
                            className="bg-transparent text-slate-900 dark:text-white w-full outline-none placeholder:text-slate-400" placeholder="Search for fresh catch..." />
                    </div>
                    
                    {/* NAVIGATION LINKS */}
                    <div className="space-y-4">
                        <Link to="/products" onClick={() => setMobileMenuOpen(false)} className="text-2xl font-serif text-slate-900 dark:text-white block py-2 border-b border-slate-100 dark:border-white/10">Shop All</Link>
                        <div className="grid grid-cols-2 gap-4">
                          <Link to="/products?category=Fish" onClick={() => setMobileMenuOpen(false)} className="text-lg text-slate-600 dark:text-white/70 block px-4 py-3 bg-slate-50 dark:bg-white/5 rounded-xl">Fish</Link>
                          <Link to="/products?category=Prawn" onClick={() => setMobileMenuOpen(false)} className="text-lg text-slate-600 dark:text-white/70 block px-4 py-3 bg-slate-50 dark:bg-white/5 rounded-xl">Prawns</Link>
                          <Link to="/products?category=Crab" onClick={() => setMobileMenuOpen(false)} className="text-lg text-slate-600 dark:text-white/70 block px-4 py-3 bg-slate-50 dark:bg-white/5 rounded-xl">Crabs</Link>
                        </div>
                    </div>

                    {/* USER SECTION MOBILE */}
                    {!user ? (
                        <button onClick={() => { googleLogin(); setMobileMenuOpen(false); }} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold mt-8 shadow-lg shadow-blue-600/20">Login / Signup</button>
                    ) : (
                        <div className="mt-8 pt-8 border-t border-slate-100 dark:border-white/10 space-y-2">
                            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-4">Account Settings</p>
                            <button onClick={() => { navigate('/profile'); setMobileMenuOpen(false); }} className="w-full flex items-center gap-4 text-slate-700 dark:text-white/70 py-3 text-lg"><FiUser /> My Profile</button>
                            <button onClick={() => { navigate('/orders'); setMobileMenuOpen(false); }} className="w-full flex items-center gap-4 text-slate-700 dark:text-white/70 py-3 text-lg"><FiPackage /> My Orders</button>
                            <button onClick={() => { navigate('/notifications'); setMobileMenuOpen(false); }} className="w-full flex items-center gap-4 text-slate-700 dark:text-white/70 py-3 text-lg justify-between"><div className="flex items-center gap-4"><FiBell /> Notifications</div> {unreadCount > 0 && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{unreadCount}</span>}</button>
                            <button onClick={() => { handleLogout(); setMobileMenuOpen(false); }} className="w-full flex items-center gap-4 text-red-500 py-3 text-lg font-bold mt-4"><FiLogOut /> Logout</button>
                        </div>
                    )}
                </div>
            </motion.div>
...
function DropdownItem({ icon, text, onClick, isDanger, badge }) {
  return (
    <motion.div whileHover={{ x: 4, backgroundColor: isDanger ? "rgba(239, 68, 68, 0.1)" : "rgba(37, 99, 235, 0.05)" }} onClick={onClick}
      className={`px-5 py-3 cursor-pointer flex items-center justify-between transition-colors
        ${isDanger ? "text-red-500" : "text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-white"}`}
    >
