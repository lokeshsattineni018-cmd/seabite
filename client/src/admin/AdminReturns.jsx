    ...
    <div className="p-6 min-h-screen bg-gradient-to-br from-white via-stone-50 to-white text-stone-900 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-500">Manage return approvals, verify product weights, and issue digital wallet refunds</p>
          </div>
        </div>
        <button
          onClick={fetchOrders}
        ...
        <div className="flex gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 bg-stone-50 border border-stone-200 rounded-xl text-xs text-stone-900 placeholder-slate-500 outline-none focus:border-rose-500/50"
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2].map(i => <div key={i} className="h-40 bg-stone-50 rounded-2xl animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="rounded-2xl border border-stone-100 bg-stone-50 p-12 text-center">
          ...
          <p className="text-sm text-stone-500 font-semibold mt-3">No return requests found</p>
          <p className="text-xs text-stone-400 mt-1">Return requests from customers will appear here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filtered.map(order => (
            <motion.div
              key={order._id}
              className="rounded-2xl border border-stone-100 bg-white border border-stone-200 shadow-sm p-5 space-y-4 hover:border-stone-200 transition-all flex flex-col justify-between"
            ...
            >
              <div>
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-sm font-bold text-stone-900 flex items-center gap-2">
                      Order #{order.orderId}
                      <span className="px-2 py-0.5 rounded-full text-[9px] bg-slate-800 text-stone-500 font-semibold uppercase">
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 bg-stone-100 rounded-xl p-3 text-xs">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-bold text-stone-400 block">Refund Amount</span>
                ...
                <div className="mt-3 space-y-1">
                  <span className="text-[10px] uppercase font-bold text-stone-400 block">Reason for Return</span>
                  <p className="text-xs text-stone-700 bg-stone-50 rounded-xl p-3 border border-stone-100 leading-relaxed">
