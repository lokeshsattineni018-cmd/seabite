...
// ✅ NEW: Dashboard Skeleton to stop white flashes
const DashboardSkeleton = () => (
  <div className="space-y-8 animate-pulse">
    <div className="flex justify-between items-end"><div className="space-y-2"><div className="h-8 w-64 bg-slate-200 rounded" /><div className="h-4 w-40 bg-slate-100 rounded" /></div></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[...Array(3)].map((_, i) => <div key={i} className="h-32 bg-white border border-slate-100 rounded-2xl" />)}
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 h-[400px] bg-white border border-slate-100 rounded-2xl" />
      <div className="h-[400px] bg-white border border-slate-100 rounded-2xl" />
    </div>
  </div>
);
...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    // Keep UI visible on subsequent filter changes
    try {
      const [dashboardRes, messagesRes, reviewsRes] = await Promise.all([
        axios.get("/api/admin", {
          params: { range: timeFilter },
          withCredentials: true // Added to fix 403 issues
        }),
  ...
  // ✅ UPDATED: Use Skeleton instead of full screen spinner
    ...
    <div className="p-4 md:p-10 bg-slate-50 min-h-screen space-y-6 md:space-y-8 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-xs md:text-sm italic">
            Authenticated via MongoDB Session Store
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-6">
          ...
          <div className="space-y-4">
            {recentMessages?.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-6">No messages.</p>
            ) : (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-6">
          ...
          <div className="space-y-4">
            {allReviews?.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-6">No reviews.</p>
            ) : (
              allReviews?.map((rev) => (
...
function StatCard({ title, value, icon, trend, lightColor }) {
  return (
    <motion.div whileHover={{ y: -1 }} className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100">
      <div className="flex justify-between items-start mb-3 md:mb-4"><div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center ${lightColor}`}>{icon}</div><span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full flex items-center gap-1"><FiTrendingUp size={12} /> {trend}</span></div>
...
function StatusPill({ status }) {
  const styles = {
    ...
    Shipped: "text-indigo-707 bg-indigo-50 border-indigo-100",
