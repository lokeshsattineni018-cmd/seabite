        ...
        <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center">
            ...
            <button
              ...
              className="text-xs md:text-sm font-medium text-blue-600"
            >
              View All
            </button>
          </div>
              ...
              <tbody className="divide-y divide-slate-100 text-xs md:text-sm">
                {recentOrders?.map((o) => (
                  <tr key={o._id} className="hover:bg-slate-50/50">
                    <td className="px-4 py-3 font-mono font-medium text-slate-600 text-[10px]">
                      #{o.orderId || o._id.substring(0, 8)}
                    </td>
          ...
          <div className="space-y-4">
            {recentMessages?.length === 0 ? (
              ...
              recentMessages?.map((msg) => (
                <div
                  key={msg._id}
                  className="flex gap-3 items-start p-3 hover:bg-slate-50 rounded-xl border border-transparent hover:border-slate-100 cursor-pointer"
                  onClick={() => navigate("/admin/messages")}
                >
          ...
          <div className="space-y-4">
            {allReviews?.length === 0 ? (
              ...
              allReviews?.map((rev) => (
                <div
                  key={rev._id}
                  className="p-3 bg-slate-50 rounded-xl relative group border border-slate-100"
                >
                  <button
                    onClick={() =>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-3 md:mb-4">
        <div
          className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center ${lightColor}`}
        >
          {icon}
        </div>
      ...
      <div>
        <p className="text-slate-500 text-[10px] font-semibold uppercase tracking-wide mb-1">
          {title}
        </p>
        <h4 className="text-xl md:text-3xl font-bold text-slate-900">
          {value}
        </h4>
      </div>
    </motion.div>
