...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
        ...
        setAllReviews(reviewsRes.data?.slice(0, 6) || []); // 🟢 Set latest 6 reviews
  ...
  return (
    <div className="p-4 md:p-10 bg-slate-50 min-h-screen space-y-6 md:space-y-8 font-sans">
      ...
      {/* ROW 3: RECENT ORDERS, MESSAGES & REVIEWS */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-100 text-xs md:text-sm">
                    ...
                    <tr key={o._id} className="hover:bg-slate-50/50">
                        <td className="px-4 py-3 font-mono font-medium text-slate-600 text-[10px]">#{o.orderId}</td>
                        ...
                        <div key={msg._id} className="flex gap-3 items-start p-3 hover:bg-slate-50 rounded-xl border border-transparent hover:border-slate-100 cursor-pointer" onClick={() => navigate('/admin/messages')}>
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="flex justify-between items-baseline mb-0.5">
                                    <h4 className="text-[11px] font-bold text-slate-900 truncate">{msg.email.split('@')[0]}</h4>
        ...
        {/* 🟢 NEW: FEEDBACK MODERATION SECTION */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-6">
            <h3 className="text-base md:text-lg font-bold text-slate-900 mb-6">Feedback Moderation</h3>
            <div className="space-y-4">
                {allReviews?.length === 0 ? (
                    <p className="text-sm text-slate-400 text-center py-6">No reviews found.</p>
                ) : (
                    allReviews?.map((rev) => (
