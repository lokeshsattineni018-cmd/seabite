...
export default function AdminLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    if (!email || !password) {
      setModal({
        show: true,
        message: "Credential injection required.",
        type: "error",
      });
      return;
    }
    ...
    if (password.length < 8) {
      setModal({
        show: true,
        message: "Secure key must be at least 8 characters.",
        type: "error",
      });
      return;
    }
    ...
    if (window.location.protocol !== "https:") {
      setModal({
        show: true,
        message: "Admin terminal must be accessed over HTTPS.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (res.data.user.role !== "admin") {
        setModal({
          show: true,
          message: "Unauthorized Entity. Access Terminated.",
          type: "error",
        });
        return;
      }
      ...
      let msg = "Authentication mismatch. Verify credentials.";

      if (nextFail >= 5) {
        setLockedUntil(Date.now() + 60_000);
        msg =
          "Multiple failed attempts detected. Terminal temporarily locked for 60 seconds.";
      }
      ...
      >
        <div className="bg-white/70 backdrop-blur-2xl rounded-[3rem] shadow-2xl shadow-blue-900/10 border border-white p-10 md:p-12 relative overflow-hidden">
          ...
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-serif font-black text-slate-900 tracking-tight uppercase">
              Terminal <span className="text-blue-600 italic">v2.0</span>
            </h1>
            <p className="text-slate-400 text-[10px] font-black tracking-[0.4em] uppercase mt-2">
              Administrative Node Access
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-slate-900 text-white rounded-2xl py-5 font-black uppercase tracking-[0.3em] text-[10px] shadow-2xl hover:bg-blue-600 hover:shadow-blue-500/30 active:scale-[0.98] transition-all duration-500 flex items-center justify-center gap-3 mt-4 disabled:opacity-60"
            >
              {loading ? (
                <>
