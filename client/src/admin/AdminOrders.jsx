...
export default function AdminOrders() {
  ...
  const updateStatus = async (id, status) => {
    ...
    if (orderToUpdate?.status.includes("Cancelled")) {
      setModal({
        show: true,
        message: "Action Restricted: Status cannot be modified.",
        type: "error",
      });
      return;
    }
  ...
  const updateRefundStatus = async (id, refundStatus) => {
    try {
      ...
      if (selectedOrder)
        setSelectedOrder((prev) => ({ ...prev, refundStatus }));
      fetchOrders();
    } catch (err) {
      setModal({
        show: true,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-xs md:text-sm mt-1">
            Manage orders, refunds, and user feedback.
          </p>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <div className="relative flex-1 md:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white outline-none text-sm transition-all"
            />
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {filteredOrders.map((o) => (
              ...
              >
                <td className="py-4 px-6 font-mono font-medium text-slate-600 group-hover:text-blue-600">
                  #{o.orderId}
                </td>
                ...
                >
                  <select
                    ...
                    className="border rounded-lg p-1 text-[10px] font-bold outline-none"
                  >
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-500 text-xs mt-1">
              Review and moderate all customer testimonials.
            </p>
          </div>
          <button
            onClick={fetchAllReviews}
            className="p-2 bg-white rounded-lg border text-slate-500"
          >
            <FiRefreshCw
            ...
            {allReviews.map((rev) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-[10px] uppercase">
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      >
        <div className="flex justify-between items-center px-6 md:px-8 py-5 border-b sticky top-0 bg-white z-10">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 bg-slate-100 rounded-xl text-slate-700"
            >
              <FiPrinter size={18} />
            </button>
            <button
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
            ...
            {canAutoRefund && (
              ...
              >
                ...
                {order.refundStatus === "Processing" ? (
                  <button
                    onClick={() =>
                      updateRefundStatus(order._id, "Success")
                    }
          ...
          <div className="space-y-3">
            ...
            {order.items.map((item, index) => (
              <div
                key={index}
                className="flex flex-col bg-white p-3 rounded-xl border border-slate-100 gap-3"
              >
        ...
        <div className="px-6 md:px-8 py-5 bg-slate-50 border-t">
          ...
          <button
            onClick={onClose}
            className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold text-xs uppercase transition-all active:scale-[0.98]"
          >
            Exit Console
          </button>
        </div>
