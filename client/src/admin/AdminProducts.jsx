  ...
  const fetchProducts = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/api/admin/products?search=${search}`,
    ...
    <div className="p-4 md:p-10 bg-slate-50 min-h-screen relative font-sans">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              {loading ? (
                  ...
                  products.map((p) => (
                    <tr key={p._id} className="hover:bg-slate-50/80 transition-colors group">
                        <td className="py-4 px-6">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-lg bg-slate-50 overflow-hidden border border-slate-200 shrink-0 p-1">
                                   <img src={p.image.startsWith('http') ? p.image : `${API_URL}/uploads/${p.image.split(/[/\\]/).pop()}`} alt="" className="w-full h-full object-contain" />
                                </div>
                        ...
                        <td className="py-4 px-6">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black border uppercase tracking-widest
                                ${p.stock === 'in' ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-red-50 text-red-700 border-red-100"}`}>
                        ...
                        <td className="py-4 px-6 text-right">
                            <div className="flex items-center justify-end gap-1">
                                <Link to={`/admin/edit-product/${p._id}`}>
                                    <button className="p-2 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all"><FiEdit2 size={16} /></button>
                                </Link>
        ...
        <div className="md:hidden divide-y divide-slate-100">
           {loading ? (
             <div className="py-20 text-center"><div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto"></div></div>
           ) : (
             products.map((p) => (
               <div key={p._id} className="p-4 flex items-center gap-4 active:bg-slate-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                         <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-1.5 rounded uppercase tracking-tighter">{p.category}</span>
                         <span className={`text-[10px] font-bold ${p.stock === 'in' ? 'text-emerald-600' : 'text-red-600'}`}>• {p.stock === 'in' ? 'Available' : 'Sold Out'}</span>
                      </div>
                      <p className="mt-1 font-mono font-bold text-xs text-slate-700">{formatPrice(p.basePrice)} <span className="text-[9px] font-sans opacity-50">/ {p.unit}</span></p>
                  </div>
                  <div className="flex flex-col gap-2">
                      <Link to={`/admin/edit-product/${p._id}`} className="p-2 bg-slate-100 rounded-lg text-slate-600"><FiEdit2 size={14}/></Link>
                      <button onClick={() => deleteProduct(p._id)} className="p-2 bg-red-50 rounded-lg text-red-600"><FiTrash2 size={14}/></button>
                  </div>
               </div>
             ))
           )}
