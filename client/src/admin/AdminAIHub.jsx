    ...
    <div className="p-6 min-h-screen bg-[#0a1625] text-slate-100 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Algorithmic intelligence, anomaly feeds, forecasting and smart reorders</p>
          </div>
        </div>
        <button
          onClick={fetchData}
        ...
        {[
          ...
          >
            ...
            {tab.count !== undefined && tab.count > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold ${activeTab === tab.id ? "bg-white text-violet-600" : "bg-white/10 text-slate-300"}`}>
                {tab.count}
              </span>
            )}
          </button>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-xs text-slate-400">Synthesizing predictions and training local ML models...</p>
        </div>
      ) : (
        <AnimatePresence mode="wait">
          ...
          {activeTab === "anomalies" && (
            ...
            >
              <div className="lg:col-span-2 space-y-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-2">Live Anomaly Feed</h3>
                {anomalies.length === 0 ? (
                  <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-8 text-center">
                    <span className="text-3xl">🛡️</span>
                    <p className="text-sm text-slate-300 font-semibold mt-3">All Systems Nominal</p>
                    <p className="text-xs text-slate-500 mt-1">No deviations, spikes, or resource anomalies detected in the last 24h.</p>
                  </div>
                ) : (
                  anomalies.map((anom, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-300 mt-1 leading-relaxed">{anom.description}</p>
                        {anom.products && (
                          <div className="mt-3 flex flex-wrap gap-1.5">
                            {anom.products.map((p, pIdx) => (
                              <span key={pIdx} className="px-2 py-1 bg-white/5 border border-white/5 rounded-lg text-[10px] text-slate-400">
              ...
              <div className="space-y-6">
                <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-5">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Guardrail Overview</h4>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs py-2 border-b border-white/5">
                      <span className="text-slate-400">Resource Guard</span>
                      <span className="text-emerald-400 font-bold">NOMINAL</span>
                    </div>
                    <div className="flex items-center justify-between text-xs py-2 border-b border-white/5">
                      <span className="text-slate-400">Cold Chain Monitor</span>
                      <span className="text-emerald-400 font-bold">ACTIVE</span>
                    </div>
                    <div className="flex items-center justify-between text-xs py-2 border-b border-white/5">
                      <span className="text-slate-400">API Telemetry Rate</span>
                      <span className="text-slate-300 font-mono">1.2ms latency</span>
                    </div>
                    <div className="flex items-center justify-between text-xs py-2">
                      <span className="text-slate-400">Fraud Sentinel</span>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Projected 30D Revenue</span>
                  <span className="text-2xl font-bold text-white mt-1 block">₹{(forecast.projected30dRevenue || 0).toLocaleString()}</span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Average Daily Sales</span>
                  <span className="text-2xl font-bold text-white mt-1 block">₹{(forecast.avgDailyRevenue || 0).toLocaleString()}</span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Average Daily Orders</span>
                  <span className="text-2xl font-bold text-white mt-1 block">{forecast.avgDailyOrders} orders</span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Demand Trend Direction</span>
              ...
              {/* Chart */}
              <div className="bg-slate-900/60 border border-white/5 rounded-3xl p-6 h-[400px]">
                <h3 className="text-sm font-bold text-white mb-6">Historical vs AI Predicted Demand (30 Days Forward)</h3>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-white/5 bg-white/[0.02]">
                      <th className="p-4 text-slate-400 font-bold uppercase">Product</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Stock Level</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Threshold</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">30D Velocity</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Est. Stockout</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Urgency</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Suggested PO</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reorders.suggestions.map((p, idx) => (
                      <tr key={idx} className="border-b border-white/5 hover:bg-white/[0.01] transition-colors">
                        ...
                        <td className="p-4 font-mono text-slate-300">{p.countInStock} kg</td>
                        <td className="p-4 font-mono text-slate-400">{p.stockThreshold} kg</td>
                        ...
                        <td className="p-4 font-bold text-violet-400">
                          {p.suggestedReorderQty} kg <span className="text-[10px] text-slate-500 font-normal">(₹{p.estimatedCost.toLocaleString()})</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-5">
                  ...
                  <span className="text-3xl font-bold text-white mt-1 block">{churnData.critical}</span>
                </div>
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5">
                  <span className="text-[10px] text-amber-400 font-bold uppercase tracking-wider block">High Risk Customers</span>
                  <span className="text-3xl font-bold text-white mt-1 block">{churnData.high}</span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">At Risk Revenue (Monthly)</span>
                  <span className="text-3xl font-bold text-white mt-1 block">₹{Math.round(churnData.potentialRevenueLoss).toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="overflow-x-auto rounded-2xl border border-white/5 bg-slate-900/40">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-white/5 bg-white/[0.02]">
                      <th className="p-4 text-slate-400 font-bold uppercase">Customer</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">LTV / Orders</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Last Order</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Inactivity</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Risk Score</th>
                      <th className="p-4 text-slate-400 font-bold uppercase">Suggested Playbook</th>
                    </tr>
                  </thead>
                  <tbody>
                    {churnData.users.map((u, idx) => (
                      <tr key={idx} className="border-b border-white/5 hover:bg-white/[0.01] transition-colors">
                        <td className="p-4">
                          ...
                          <p className="text-[10px] text-slate-500 mt-0.5">{u.email || u.phone}</p>
                        </td>
                        <td className="p-4">
                          <p className="font-mono text-slate-300">₹{(u.lifetimeOrderValue || 0).toLocaleString()}</p>
                          <p className="text-[10px] text-slate-500 mt-0.5">{u.lifetimeOrderCount || 0} orders</p>
                        </td>
                        <td className="p-4 font-mono text-slate-300">
                        ...
                        <td className="p-4 text-slate-300 font-semibold">{u.suggestedAction}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            >
              <div className="space-y-6">
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">Sentiment Distribution</h4>
                  ...
                  <div className="flex items-center justify-between text-xs mt-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-slate-400">Positive ({sentiment.overview.positive})</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-3 h-3 bg-slate-500 rounded-full" />
                      <span className="text-slate-400">Neutral ({sentiment.overview.neutral})</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-3 h-3 bg-red-500 rounded-full" />
                      <span className="text-slate-400">Negative ({sentiment.overview.negative})</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-2">Attention Required (Poor Rating Trends)</h3>
                {sentiment.flaggedProducts.length === 0 ? (
                  <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-8 text-center">
                    <span className="text-3xl">🤩</span>
                    <p className="text-sm text-slate-300 font-semibold mt-3">High Product Satisfaction</p>
                    <p className="text-xs text-slate-500 mt-1">No products have high rates of negative feedback currently.</p>
                  </div>
                ) : (
                  sentiment.flaggedProducts.map((p, idx) => (
                    <div key={idx} className="bg-white/[0.02] border border-red-500/10 rounded-2xl p-5 space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="text-sm font-bold text-white">{p.name}</h4>
                          <p className="text-[10px] text-slate-500 mt-0.5">Avg. Rating: {p.avgRating} • {p.totalReviews} total reviews</p>
                        </div>
          ...
          {activeTab === "generator" && (
            ...
            >
              <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
                <h3 className="text-sm font-bold text-white mb-6">Gemini Description Generator</h3>
                <form onSubmit={handleGenerateDescription} className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-600 outline-none focus:border-violet-500/50"
                      required
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Base Price (INR)</label>
                      <input
                        type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Key Features / Bullet Points</label>
                    <textarea
                      ...
                      rows={3}
                      className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-600 outline-none focus:border-violet-500/50 resize-none"
                    />
                  </div>
                  <button
                    type="submit"
                    disabled={generatingDesc}
                    className="w-full py-3 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white font-bold rounded-xl text-xs transition-colors flex items-center justify-center gap-2"
                  >
                    {generatingDesc ? (
                      <>
              ...
              <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 flex flex-col min-h-[350px]">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Output Preview</h4>
                {generatedDesc ? (
                  <div className="flex-1 flex flex-col justify-between">
                    <p className="text-xs text-slate-200 leading-relaxed font-sans whitespace-pre-line">{generatedDesc}</p>
                    <button
                      onClick={() => {
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-xs text-slate-500 mt-2">Outputs generated by Gemini will appear here.</p>
                  </div>
                )}
              </div>
            </motion.div>
