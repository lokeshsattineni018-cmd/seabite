...
import { FiUploadCloud, FiX, FiCheck, FiDollarSign, FiType, FiLayers, FiTrendingUp, FiBox, FiPlus, FiAlertCircle, FiImage } from "react-icons/fi";
...
export default function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.category || !form.basePrice || !form.unit) {
      setModal({ show: true, message: "Please fill in all required fields.", type: "error" });
      setLoading(false);
      return;
    }
        ...
        <div className="lg:col-span-1 space-y-4 md:space-y-6">
          ...
          <div className="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
             <div className="flex items-center gap-3">
               ...
               <div>
                 <p className="font-bold text-xs md:text-sm text-slate-900">Trending Item</p>
                 <p className="text-[10px] text-slate-500">Highlight on homepage</p>
               </div>
             </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div 
                  ...
                  className={`cursor-pointer rounded-xl p-3 md:p-4 border flex items-center justify-center gap-2 transition-all ${form.stock === "in" ? "border-emerald-500 bg-emerald-50 text-emerald-700" : "border-slate-200 hover:bg-slate-50 text-slate-500"}`}
                >
                ...
                <div 
                  ...
                  className={`cursor-pointer rounded-xl p-3 md:p-4 border flex items-center justify-center gap-2 transition-all ${form.stock === "out" ? "border-red-500 bg-red-50 text-red-700" : "border-slate-200 hover:bg-slate-50 text-slate-500"}`}
                >
